...
\begin{tikzpicture}[
    ...
    \definecolor{col1}{RGB}{102, 194, 165} % Green (Now F2 - Train)
    \definecolor{col2}{RGB}{217, 95, 2}    % Orange (Now F1 - Test, Emphasized)
    \definecolor{col3}{RGB}{141, 160, 203} % Purple (F3 - Train)


    \def\totalwidth{14}  % 总宽度
    \def\proxystart{4}   % l
    \def\proxyend{10}    % r
    \def\sep{0.5}        % 间距

    % ==================================================
    % 1. Order-Preserved Split
    % ==================================================
    \def\yop{0}
    ...
    % \node[circle, fill=black, inner sep=1.5pt, label=below:{$l_{end}$}] at (0, \yop) {};
    % \node[circle, fill=black, inner sep=1.5pt, label=below:{$r_{end}$}] at (\totalwidth, \yop) {};
    ...
    % \node[blue, font=\bfseries, anchor=south east] at (\proxystart, \yop - 0.5) {$l$};
    % \node[blue, font=\bfseries, anchor=south west] at (\proxyend, \yop - 0.5) {$r$};
    ...
    \foreach \i in {0, ..., 28} {
        ...
        % [Color Swap] F1 is Test (Orange/col2), F2 is Train (Green/col1)
        \ifnum\foldid=1
            % Test (F1) - Orange (col2)
            \node[data_point, fill=col2!100, draw=col2!100] at (\xpos, \yop + 0.4) {};
        \else
            % Train (F2, F3) depends on Proxy
            \ifnum\inproxy=1
    ...
    % % [修正] Part 1 图例 (Updated Colors)
    % \node[anchor=west, align=left, font=\scriptsize, inner sep=0pt] at (\totalwidth + 1.5, \yop + 0.5) {
    %     \legendBox{fill=col2!100, draw=col2!100} \textbf{Test (F1)}: in Full $I$\\
    %     \legendBox{fill=col1!70, draw=col1!100} \textbf{Train (F2/3)}: In Proxy $R$\\
    ...
    % \node[circle, fill=black, inner sep=1.5pt, label=below:{$l_{end}$}] at (0, \yblk) {};
    % \node[circle, fill=black, inner sep=1.5pt, label=below:{$r_{end}$}] at (\totalwidth, \yblk) {};
    ...
    % \node[circle, fill=black, inner sep=1.5pt, label=below:{$l_{end}$}] at (0, \ybuf) {};
    % \node[circle, fill=black, inner sep=1.5pt, label=below:{$r_{end}$}] at (\totalwidth, \ybuf) {};
        ...
        \ifdim \drawstart pt < \drawend pt
            ...
            \else
                \ifnum\inproxy=1
                    ...
                    % Buffer Logic (Neighbors of F1)
                    % F3 is LEFT neighbor of F1.
                    \ifnum\foldid=3
    ...
    % ==================================================
    % 4. 底部图例 (Global Legend) - [新增]
    % ==================================================
    % 位置计算：放在总宽度的一半处 (\totalwidth/2)，高度在最底部的图 (\ybuf) 下方
    \node[anchor=north, align=center, font=\scriptsize, inner sep=0pt] at (\totalwidth/2, \ybuf - 0.5) {
        % 第一行
        \legendBox{fill=col2!100, draw=col2!100} \textbf{Test (F1)}: in Full $I$
        \hspace{1.5em} % 控制两个图例之间的间距
